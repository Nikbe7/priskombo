...
export default function ProductView({ product }: { product: ProductDetails }) {
  ...
  // Enkel funktion för att rendera brödsmulor rekursivt
  ...
  const handleAddToCart = () => {
    // Lägg till i inköpslista (Context)
    addToBasket({
      id: product.id,
      name: product.name,
      ean: product.ean,
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="bg-gray-50 p-10 flex items-center justify-center border-b md:border-b-0 md:border-r border-gray-100 relative">
            <Link
              href="/"
              className="absolute top-4 left-4 text-gray-400 hover:text-gray-600 transition text-sm font-medium"
            >
              ← Tillbaka
            </Link>

            {/* Här använder vi din placeholder-logik eller ProductImage om du vill byta senare */}
            {product.image_url ? (
          ...
          <div className="p-8 md:p-12 flex flex-col justify-center">
            ...
            {/* --- 3. UPPDATERA KNAPPEN HÄR (använd handleAddToCart) --- */}
            <button
            ...
            <div className="text-xs text-gray-400">EAN: {product.ean}</div>
          </div>
        </div>
        ...
        <div className="p-8 bg-gray-50 border-t border-gray-100">
          <h3 className="font-bold text-gray-800 mb-4 text-lg">
            Jämför alla butiker ({product.prices.length})
          </h3>
          <div className="space-y-3">
            {product.prices.map((offer, idx) => (
              <div
                key={idx}
                className="flex justify-between items-center p-4 border rounded-lg hover:border-blue-300 transition bg-white relative overflow-hidden group"
              >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-400">
                      {offer.base_shipping !== undefined
                        ? offer.base_shipping === 0
                          ? "Fri frakt"
                          : `+ ${offer.base_shipping} kr frakt`
                        : ""}
                    </div>
                  </div>
